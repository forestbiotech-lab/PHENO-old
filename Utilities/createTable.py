...
def generateTable(table,output):
  ...
  if table=="DataTypes":
    my_dict={'DataTypes':'DTypes'}
  else:
    my_dict['DataTypes']='DataTypes'
  result="""/**
  ...
  module.exports = function(sequelize, %(DataTypes)s) {
    const %(table)s = sequelize.define('%(table)s', {
      id: { 
        type: %(DataTypes)s.INTEGER(11),
        autoIncrement: true,
        primaryKey: true,
        allowNull: false,
        unique: true,
  ...
  for attribute, datatype in tableAttributes[1::]:
    ...
    if table=="DataTypes":
      result+="    %(attribute)s: DTypes.%(datatype)s,\n" % attr
    else:
